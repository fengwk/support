...
class EmailPasswordLogin extends PureComponent {
  ...
  render() {
    ...
    const { changeEmail, changePassword, loginByRandom, registerOrLogin, forgetPwd } = this.props;
    ...
    return (
      <div>
        <Input
          ...
          onPressEnter={() => { registerOrLogin(history, param, this.emailInput, this.pwdInput); }}
        ...
        <Button 
          ...
          type="primary" 
          block
          onClick={() => { registerOrLogin(history, param, this.emailInput, this.pwdInput); }}
        >
          注册/登录
        </Button>
      </div>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  registerOrLogin(history, param, emailInput, pwdInput) {
    dispatch(actionCreators.registerOrLoginByEmailAndPassword(history, param, emailInput, pwdInput));
  },
