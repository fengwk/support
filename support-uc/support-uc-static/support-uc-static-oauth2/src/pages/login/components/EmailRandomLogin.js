...
class EmailRandomLogin extends PureComponent {
  ...
  render() {
    ...
    const { changeEmail, changeRandom, sendRandom, registerOrLogin } = this.props;
    ...
    return (
      <div>
        <Input
          ...
          onPressEnter={() => { registerOrLogin(history, param, this.emailInput, this.randomInput); }}
        ...
        <Button 
          ...
          type="primary" 
          block
          onClick={() => { registerOrLogin(history, param, this.emailInput, this.randomInput); }}
        >
          注册/登录
        </Button>
      </div>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  registerOrLogin(history, param, emailInput, randomInput) {
    dispatch(actionCreators.registerOrLoginByEmailAndRandom(history, param, emailInput, randomInput));
  }
})
